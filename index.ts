...
class SiteAccessChatWidget {
  ...
  private callState: {
    callId: string | null;
    status: 'idle' | 'ringing' | 'in_call' | 'ended' | 'error';
    ...
    try {
      ...
      this.socket.on('call:offer', (data: any) => {
        if (data.fromRole === 'operator') {
          ...
          this.callState.incomingCall = { callId: data.callId, fromRole: 'operator', kind: data.kind, sdp: data.sdp };
          this.callState.status = 'ringing';
      ...
      this.socket.on('call:ice', async (data: any) => {
        if (data.callId === this.callState.callId && data.candidate && this.peerConnection) {
          try {
  ...
  private async fetchIceServers(): Promise<RTCIceServer[]> {
    ...
    try {
      const response = await fetch(`${this.apiBase}/api/calls/ice`, {
        headers: {
          Authorization: `Bearer ${this.visitorSessionToken}`,
        },
      });
      ...
      console.log(`[CALLS] Fetched ${this.iceServers.length} ICE servers`);
      return this.iceServers;
    } catch (error: any) {
  ...
  private async handleAcceptCall() {
    ...
    try {
      ...
      const kind = this.callState.incomingCall.kind as 'audio' | 'video';
      ...
      if (this.callState.incomingCall.sdp) {
        await pc.setRemoteDescription(new RTCSessionDescription(this.callState.incomingCall.sdp));
      }
  ...
  private renderIncomingCall() {
    ...
    } else if (this.callState.status === 'error') {
      if (!callPanel) {
        callPanel = document.createElement('div');
        callPanel.className = 'incoming-call-panel';
        callPanel.style.cssText = 'padding: 12px; background: #f8d7da; border-bottom: 1px solid #ddd; text-align: center; color: #721c24;';
        this.panel.insertBefore(callPanel, this.messagesContainer);
      }
      callPanel.innerHTML = `<div>Error: ${this.callState.error || 'Unknown error'}</div>`;
    } else {
      if (callPanel) {
        callPanel.remove();
      }

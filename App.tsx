...
function App() {
  ...
  const selectedConversationRef = useRef<string | null>(null); // For sync after reconnect
  const maxRetries = 5;
  ...
  const [callState, setCallState] = useState<{
    callId: string | null;
    status: 'idle' | 'calling' | 'ringing' | 'in_call' | 'ended' | 'error';
    kind: 'audio' | 'video' | null;
    incomingCall: { callId: string; fromRole: string; kind: string; sdp?: string } | null;
    error?: string;
  }>({
    callId: null,
    status: 'idle',
  ...
  const handleConnectWithToken = async (token: string, chId: string) => {
    ...
    try {
      ...
      ws.on('call:offer', (data: any) => {
        if (data.conversationId === selectedConversation && data.fromRole === 'visitor') {
          setCallState(prev => ({
            ...prev,
            callId: data.callId,
            incomingCall: { callId: data.callId, fromRole: 'visitor', kind: data.kind, sdp: data.sdp },
            status: 'ringing',
            kind: data.kind,
          }));
        }
      ...
      ws.on('call:answer', async (data: any) => {
        if (data.callId === callState.callId && peerConnectionRef.current) {
          try {
            console.log('[CALLS] Received call:answer, setting remote description');
            ...
            for (const candidate of pendingIceCandidatesRef.current) {
              try {
                ...
                console.warn('[CALLS] Failed to add pending ICE candidate:', e);
              }
            }
            pendingIceCandidatesRef.current = [];
          } catch (error: any) {
            console.error('[CALLS] Failed to set remote description:', error);
            setCallState(prev => ({ ...prev, status: 'error', error: error.message }));
          }
        }
      });
      
      ws.on('call:ice', async (data: any) => {
        if (data.callId === callState.callId && data.candidate) {
          try {
            if (peerConnectionRef.current) {
              if (peerConnectionRef.current.remoteDescription) {
                await peerConnectionRef.current.addIceCandidate(new RTCIceCandidate(data.candidate));
                console.log('[CALLS] Added ICE candidate');
              } else {
                // Queue candidate if remote description not set yet
                pendingIceCandidatesRef.current.push(new RTCIceCandidate(data.candidate));
                console.log('[CALLS] Queued ICE candidate (remote description not set)');
              }
            }
          } catch (error: any) {
            console.warn('[CALLS] Failed to add ICE candidate:', error);
          }
        }
      });
  ...
  const fetchIceServers = async (): Promise<RTCIceServer[]> => {
    ...
    try {
      const response = await fetch(`${API_URL}/api/calls/ice`, {
        headers: {
          Authorization: `Bearer ${operatorToken}`,
        },
      });
      ...
      console.log(`[CALLS] Fetched ${iceServersRef.current.length} ICE servers`);
      return iceServersRef.current;
    } catch (error: any) {
      console.error('[CALLS] Failed to fetch ICE servers:', error);
  ...
  const handleStartCall = async (kind: 'audio' | 'video') => {
    if (!socket || !selectedConversation || !operatorChannelId) {
      setError('Cannot start call: missing connection or conversation');
      return;
    }

    try {
      setCallState(prev => ({ ...prev, status: 'calling', kind }));
      ...
      setCallState(prev => ({ ...prev, callId, status: 'calling' }));
      
      // Handle ICE candidates
      pc.onicecandidate = (event) => {
        if (event.candidate && socket) {
          const candidate = event.candidate.toJSON();
          console.log(`[CALLS] Sending ICE candidate (type: ${candidate.type}, length: ${JSON.stringify(candidate).length})`);
      ...
      pc.onconnectionstatechange = () => {
        console.log(`[CALLS] Connection state: ${pc.connectionState}`);
        if (pc.connectionState === 'failed' || pc.connectionState === 'disconnected') {
          setCallState(prev => ({ ...prev, status: 'error', error: `Connection ${pc.connectionState}` }));
        }
      };
      ...
      // Send offer
      console.log(`[CALLS] Sending call:offer (type: ${offer.type}, length: ${offer.sdp?.length || 0})`);
      ...
      setCallState(prev => ({ 
        ...prev, 
        status: 'error', 
        error: error.message || 'Failed to start call. Check microphone permissions.' 
      }));
      cleanupCall();
    }
  };
  ...
  const handleAcceptCall = async () => {
    if (!socket || !selectedConversation || !operatorChannelId || !callState.incomingCall) {
      return;
    }

    try {
      const callId = callState.incomingCall.callId;
      const kind = callState.incomingCall.kind as 'audio' | 'video';
      ...
      if (callState.incomingCall.sdp) {
        await pc.setRemoteDescription(new RTCSessionDescription(callState.incomingCall.sdp));
      }
      ...
      setCallState(prev => ({ ...prev, callId, status: 'in_call' }));
      
      // Send answer
      console.log(`[CALLS] Sending call:answer (type: ${answer.type}, length: ${answer.sdp?.length || 0})`);
      ...
      setCallState(prev => ({ 
        ...prev, 
        status: 'error', 
        error: error.message || 'Failed to accept call. Check microphone permissions.' 
      }));
      cleanupCall();
    }
  };
  ...
  const handleHangup = () => {
    if (!socket || !selectedConversation || !operatorChannelId || !callState.callId) {
      return;
    }
